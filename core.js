...
(function(){
  ...
  function saveArtists(){
    localStorage.setItem(ARTIST_KEY,JSON.stringify(state.artists||[]));
    document.dispatchEvent(new CustomEvent('tatnera:artists-changed',{detail:{artists:getArtists(false)}}));
  }

  function getArtists(activeOnly=true){
    const list=Array.isArray(state.artists)?state.artists:[];
  ...
  function addArtist(name){
    ...
    if(existing){existing.active=true;saveArtists();return existing;}
    const artist={id:'artist-'+Date.now(),name:clean,active:true};state.artists.push(artist);saveArtists();return artist;
  }

  function setArtistActive(id,active){
    const artist=(state.artists||[]).find(item=>item.id===id);if(!artist)return false;
    artist.active=Boolean(active);saveArtists();return true;
  }

  function renameArtist(id,name){
    const artist=(state.artists||[]).find(item=>item.id===id);const clean=String(name||'').trim();if(!artist||!clean)return false;
  ...
  function completedTattooEvents(projectId){
    ...
    return (state.calendarEvents||[])
      .filter(event=>event.projectId===projectId&&event.type==='tattoo'&&event.date<=today)
  ...
  function activateProjectTab(name,{emit=true}={}){
    const detail=document.getElementById('projectDetail');if(!detail||!name)return false;
    let found=false;
    detail.querySelectorAll('[data-project-tab]').forEach(button=>{
      const active=button.dataset.projectTab===name;found=found||active;button.classList.toggle('active',active);button.setAttribute('aria-selected',active?'true':'false');button.type='button';
    });
  ...
  window.TatneraCore={
    esc,getArtists,saveArtists,addArtist,setArtistActive,renameArtist,artistNameFallback,populateArtistSelect,
    projectIdFromDetail,getProject,currentProject,getCustomer,completedTattooEvents,lastCompletedTattooDate,activateProjectTab
  };
})();
