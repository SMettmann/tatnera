...
(function(){
  ...
  function openView(){
    if(typeof window.navigate==='function'){
      window.navigate('tatnera-you');
      return;
    }
    document.querySelectorAll('.view').forEach(view=>view.classList.remove('active-view'));
    document.getElementById('tatnera-you')?.classList.add('active-view');
  ...
  function addDesktopNav(){
    ...
    if(!button){
      button=document.createElement('button');button.type='button';button.className='nav-item tatnera-you-nav';button.dataset.view='tatnera-you';button.innerHTML='<span>♡</span> TATNERA & Du';nav.insertBefore(button,nav.firstChild);
    }
  ...
  document.addEventListener('tatnera:auth-ready',()=>{build();setTimeout(addMobileMoreItem,100);});
