/* TATNERA — local design storage, event driven */
(function(){
  'use strict';
  ...
  function enhanceDesignPane(){
    ...
    (project.versions||[]).forEach((version,index)=>{
      ...
      const muted=tile.querySelector('.muted');if(muted)muted.textContent=version.data?'Bild lokal in dieser Tattoo-Akte gespeichert':version.type==='file'?'Dateieintrag im MVP gespeichert':'Version im MVP vermerkt';
    ...
    const upload=pane.querySelector('label.design-tile .muted');if(upload)upload.textContent='Bilder werden lokal gespeichert; PSD/PDF/Procreate zunächst als Dateieintrag.';
  }
  ...
  async function handleUpload(input,file){
    ...
    if(version.type==='image'){try{version.data=await compressImage(file);}catch(_error){alert('Das Bild konnte nicht verarbeitet werden.');input.value='';return;}}
    project.versions=project.versions||[];project.versions.push(version);
    try{persist();}catch(_error){project.versions.pop();alert('Der lokale Browserspeicher ist voll. Für größere Dateien benötigen wir später den Cloud-Dateispeicher.');input.value='';return;}
