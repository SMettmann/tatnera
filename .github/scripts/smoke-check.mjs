...
const scriptRefs=[...index.matchAll(/<script\s+[^>]*src=["']([^"']+)["'][^>]*><\/script>/gi)].map(match=>match[1]);
if(!scriptRefs.length)fail('index.html enthält keine lokalen Script-Referenzen.');
...
if(scriptRefs.at(-1)?.split(/[?#]/)[0]!=='project-tabs-runtime.js'){
  fail('project-tabs-runtime.js muss als letzter statischer Script-Controller geladen werden.');
}
...
for(const file of forbiddenLegacy){
  ...
  if(index.includes(file))fail(`index.html referenziert Legacy-Datei: ${file}`);
}
